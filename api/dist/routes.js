import { isAppAdmin, readYnhIdentity } from './sso.js';
import { rowToReview } from './review-mapper.js';
...
async function upsertUserDirectory(pool, id) {
    await pool.execute(`INSERT INTO user_directory (uid, email, display_name)
     VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE email = VALUES(email), display_name = VALUES(display_name)`, [id.uid, id.email, id.fullName]);
}
export async function registerRoutes(app, pool) {
    await app.register(async (r) => {
        ...
        r.get('/reviews', async (req, reply) => {
            ...
            if (q.search) {
                where.push('(title LIKE ? OR book_title LIKE ? OR description LIKE ? OR content LIKE ?)');
                const s = `%${q.search}%`;
                params.push(s, s, s, s);
            }
        ...
        r.get('/reviews/:id', async (req, reply) => {
            const { id } = req.params;
        ...
        r.post('/reviews', async (req, reply) => {
            ...
            const [created] = await pool.query('SELECT * FROM reviews WHERE id = ?', [id]);
            return rowToReview(created[0]);
        });
        r.put('/reviews/:id', async (req, reply) => {
            const identity = requireUser(req, reply);
            if (!identity)
                return;
            const { id } = req.params;
            ...
            const rating = body.rating != null ? Number(body.rating) : parseFloat(String(row.rating));
            ...
            const now = new Date();
            await pool.execute(`UPDATE reviews SET
        ...
        r.delete('/reviews/:id', async (req, reply) => {
            const identity = requireUser(req, reply);
            if (!identity)
                return;
            const { id } = req.params;
        ...
        r.get('/admin/users', async (req, reply) => {
            if (!requireAdmin(req, reply))
                return;
            const [rows] = await pool.query('SELECT uid, email, display_name, created_at FROM user_directory ORDER BY created_at DESC');
            return rows.map((u) => {
        ...
        r.delete('/admin/users/:id', async (req, reply) => {
            if (!requireAdmin(req, reply))
                return;
            const { id } = req.params;
            if (isAppAdmin(id)) {
            ...
            const [res] = await pool.execute('DELETE FROM user_directory WHERE uid = ?', [
                id,
            ]);
            if (!res.affectedRows) {
                return reply.code(404).send({ error: 'Not found' });
        ...
        r.get('/admin/reviews/pending', async (req, reply) => {
            if (!requireAdmin(req, reply))
                return;
            const [rows] = await pool.query('SELECT * FROM reviews WHERE is_published = 0 ORDER BY updated_at DESC');
            return rows.map(rowToReview);
        });
        r.patch('/admin/reviews/:id/approve', async (req, reply) => {
            if (!requireAdmin(req, reply))
                return;
            const { id } = req.params;
            const body = req.body ?? {};
        ...
        r.delete('/admin/reviews/:id', async (req, reply) => {
            if (!requireAdmin(req, reply))
                return;
            const { id } = req.params;
        ...
        r.get('/admin/stats', async (req, reply) => {
            if (!requireAdmin(req, reply))
                return;
            const [[reviewCounts]] = await pool.query(`SELECT
            COUNT(*) AS totalReviews,
            SUM(is_published = 1) AS publishedReviews,
            SUM(is_published = 0) AS pendingReviews
          FROM reviews`);
            const [[userCount]] = await pool.query('SELECT COUNT(*) AS totalUsers FROM user_directory');
            const [genreRows] = await pool.query('SELECT genre, COUNT(*) AS c FROM reviews WHERE is_published = 1 GROUP BY genre');
